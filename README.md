...
- [ ] `tiny-fs` 接入内核
  ...
  - [ ] 基于RVOS实现 `virtio` 块设备驱动程序
    - [ ] Qemu启动参数添加一块 virtio 块设备
    - [ ] 建立 virtio 总线的MMIO地址空间
    - [ ] 基于外部 crate `virtio-drivers` 实现 VirtIOBlock，并为其实现 BlockDevice trait
      - [ ] 实现 VirtQueue 的内存分配trait
  - [ ] 内核索引节点 `OSInode`
    - [ ] 对 `tiny-fs` crate 提供的 `Inode` 结构进一步封装，形成 `OSInode` 结构，以表示进程中一个打开的常规文件；
    - [ ] 实现用户缓冲区的抽象 `UserBuffer`，方便内核完成文件读写操作；
    - [ ] 文件的抽象 Trait `File` 提供了 `read/write` 两个接口，`OSInode` 作为块设备文件类型应该实现 `File` trait；
    - [ ] 扩展文件描述符表，让进程管理多个文件；
    - [ ] 应用访问文件的内核机制实现
      - [ ] 文件系统初始化
      - [ ] `TaskControlBlock::new/fork` 的相关操作需要考虑到对新增的文件描述符表字段的影响
      - [ ] 移除 `loader`，基于文件来加载并执行应用
      - [ ] 实现文件相关的系统调用
        - [ ] `sys_open/sys_close` 
        - [ ] `sys_read/write` 
